...
def graficador():
    ...
    # Agregar nodos y aristas desde RDFLib
    for s, p, o in g:
        # Filtrar clases y object properties
        if (o == OWL.Class) or (o == OWL.ObjectProperty):
            continue

        # Solo mostramos relaciones más legibles (URI cortas)
        s_label = s.split("#")[-1] if "#" in s else s
        o_label = o.split("#")[-1] if "#" in o else o

        # No guardamos el label de la arista
    ...
    plt.title("Grafo RDF resultante (Sistema de Producción Académico)", fontsize=14)
    plt.show()
...
# ----------------------------------------------------------------
# 3) Crear individuos (INSTANCIAS)
# ----------------------------------------------------------------

#diccionario materias base
materias_base = {
    "2": "Álgebra Lineal",
